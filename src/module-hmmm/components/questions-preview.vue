<template>
  <div class='questions-preview'>
    <el-dialog title="题目预览" :visible.sync="show" width="900px" @close="close()">
      ...
      【题目备注】：{{myData.remarks}}
      <div slot="footer">
        <el-button type="primary" @click="show=false&&close()">关闭</el-button>
      </div>
</el-dialog>
</div>
</template>
...
export default {
  ...
  filters: {
    tx (value) {
      if (value) return questionType.find(item => item.value === parseInt(value)).label + '题'
    },
    nd (value) {
      if (value) return difficulty.find(item => item.value === parseInt(value)).label
    }
  },
  data () {
    return {
